...
export default function App() {
  ...
  return (
    <div className="mx-auto w-full py-4 px-6 flex flex-col gap-4 min-h-screen md:h-screen md:min-h-0 md:max-h-screen md:overflow-hidden">
      <header className="flex justify-between items-center gap-3 shrink-0">
        <div className='flex items-center gap-4'>
          <h1 className="text-[2rem] font-bold">PyScript</h1>
          <div className="w-[1px] h-8 bg-[#aeb9ca] "></div>
          <h3 className='text-md text-[#aeb9ca]'>Playground</h3>
        </div>
        <span className={badgeClass(status.variant)}>{status.label}</span>
      </header>

      <div className="flex flex-wrap gap-2 items-center bg-white/[0.02] border border-[#2b3850] rounded-lg py-2 px-4 shrink-0">
      ...
      <main className="flex flex-col gap-4 md:flex-1 md:min-h-0 md:overflow-hidden md:grid md:grid-cols-12">
        <section className="md:col-span-7 min-h-[460px] md:min-h-0 md:flex-1 md:overflow-hidden flex flex-col md:h-full">
          <CodeEditor
            value={source}
            onChange={setSource}
